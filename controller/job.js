...
const createJobPost = async (req , res , next) => {
    try {
        ...
        const { companyName , logoUrl , title , description , salary , location , duration , locationType , skills , jobType , information } = req.body;

        if (!companyName || !logoUrl || !title || !description || !location || !duration || !locationType || !skills || !jobType || !information ) {
...
const getJobDetailsById = async(req , res , next) => {
    try {
        ...
        if(!jobId){        //then find Id is available or not
               return res.status(400).json({errorMessage : "Please Enter Valid Id"})
        }

        const jobDetails = await Job.findById(jobId);

        if(!jobDetails){   // then find corresponding job according to the given ID  (if not)
...
const updateJobDetailsById = async(req,res,next) => {
    ...
    try {
        ...
        if( !companyName|| !logoUrl || !title || !description || !salary || !location || !duration || !locationType || !skills || !jobType || !information){
        ...
        await job.updateOne(
            { _id : jobId },{
                $set: {
                     companyName, logoUrl , title , description , salary , location , duration ,locationType , skills , jobType , information             
                        }
